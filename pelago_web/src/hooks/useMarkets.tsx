...
import { generateMockMarkets, mergeRealAndMockMarkets } from '../utils/mockMarkets';
    ...
    const fetchMarkets = async () => {
      try {
        ...
        const formattedMarkets = allMarkets.map(m => ({
          ...
          totalSupplyAssets: m.account.totalSupplyAssets.toNumber(),
          totalBorrowAssets: m.account.totalBorrowAssets.toNumber(),
        ...
        // 合并真实市场和mock市场
        // 真实市场（3giB...）始终排在第一位并使用真实数据
        const mergedMarkets = mergeRealAndMockMarkets(formattedMarkets, mockMarkets);

        setMarkets(mergedMarkets as Market[]);
      } catch (err) {
